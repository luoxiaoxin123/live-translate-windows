...
| 模块 | 说明 |
|------|------|
| 字幕页 | 目标语言（官方 Live Translate 70 余种；源语言由模型自动识别）、声音来源、启动/停止、状态与实时预览 |
...
| Live API | WebSocket + `translationConfig`，对齐官方 Live Translate 协议；收到 GoAway 后自动换新连接 |
...
- Live Translate 只接收**目标语言**；源语言由模型自己识别
- 单条 Live 连接大约 10 分钟；收到服务端 GoAway 后会自动关旧连接并开新连接，悬浮窗和已有字幕会保留
